...
def tsnegridsearch():
    ...
    # perplexities = [400, 600, 800, 1000, 1200]
    perplexities = [800, 900, 1000]
    # perplexities = [200, ]
    exaggerations = [600, 650, 700, 750, 800]
    # learn_rates = [100, 200, 400, ]
    learn_rates = [750, 1000, 1500]
    # tsne_n_iters = [1_000, 2000, 3000]
    tsne_n_iters = [1000, ]
    percent_epm_train_files_to_cluster_on = 0.2
    ...
    for i, kwargs_i in enumerate(kwargs_product):
        ...
        try:
            ...
            p_i.plot_clusters_by_assignments(
                title=graph_title,
                fig_file_prefix=f'particular2{results_current_time}__{p_i.name}__',
...
def trybuild(*args, **kwargs):
    ...
    for i, kwargs_i in enumerate(kwargs_product):
        ...
        try:
            ...
            p_i.plot_clusters_by_assignments(
                title=graph_title,
                fig_file_prefix=f'Mixup21_tryingtonailitdown{results_current_time}__{p_i.name}__',
                show_now=False, save_to_file=True, figsize=graph_dimensions,
                s=0.4 if show_cluster_graphs_in_a_popup_window else 1.5,
            )
        end_build = time.perf_counter()
